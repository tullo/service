...
func TestRead(t *testing.T) {
	t.Log("Given the need to parse a directory of private key files.")
	{
		...
		t.Logf("\tTest %d:\tWhen handling a directory of keyfile(s).", testID)
		{
			...
			const keyID = "private-test"
			pk, err := ks.PrivateKey(keyID)
			if err != nil {
				t.Fatalf("\t%s\tTest %d:\tShould be able to find the key in store: %v", failed, testID, err)
			}
			t.Logf("\t%s\tTest %d:\tShould be able to find the key in store.", success, testID)
