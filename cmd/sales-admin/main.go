...
// keygen creates a x509 private key for signing auth tokens.
func keygen() error {
	...
	if err := pem.Encode(pf, &publicBlock); err != nil {
...
func gentoken(cfg database.Config, id string, privateKeyFile string) error {
	...
	if privateKeyFile == "" {
		return errors.New("gentoken command requires a private key file")
	}
